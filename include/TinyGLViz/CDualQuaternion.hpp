...
namespace TinyGLViz {
	...
	public:
		...
		//TODO:Verify this implementation
		CDualQuaternion<T_real> getInverse() const { return CDualQuaternion(m_real.getInverse(), m_dual.getConjugate()); }

		void normalize() { m_real.normalize(); }

		std::string toString() const { return "Real: " + m_real.toString() + " Dual: " + m_dual.toString(); }
		...
		CQuaternion<T_real> GetRotation() const { return m_real; }

		vec3::Vector3<T_real> GetTranslation() const {
	...
	template<typename T>
	std::ostream &operator<<(std::ostream &os, const CDualQuaternion<T> &rhs)
	{
		return os << rhs.m_real << " " << rhs.m_dual;
	}
