...
namespace TinyGLViz {
    ...
    COpenGLGeometry::COpenGLGeometry(T_indices draw_type) {
    ...
    COpenGLGeometry::~COpenGLGeometry() {
        glDeleteVertexArrays(1, &VAO);
        glDeleteBuffers(1, &VBO);
        glDeleteBuffers(1, &EBO);
    }

    bool COpenGLGeometry::updateBuffers() {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(T_vertex) * m_vertexBufferData.size(), m_vertexBufferData.data(),
                     GL_STATIC_DRAW);
    ...
    bool COpenGLGeometry::draw(Shader *shader) {
        shader->Use();
        ...
        GLint transformNorm = glGetUniformLocation(shader->Program, "normalMatrix");
