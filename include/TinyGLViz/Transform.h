...
namespace TinyGLViz {
	...
	public:
		...
		void translate(const T_real t[3]){ translate( vec3::Vector3<T_real>(t[0],t[1],t[2]) ); }
		...
		void rotate(const T_real r[4]) { rotate( CQuaternion<T_real>(r) ); }
		...
		CTransform inverse()
        {
            CDualQuaternion<T_real> res = m_data;
            res.inverse();
            return CTransform<T_real>( res );
        }
	};
